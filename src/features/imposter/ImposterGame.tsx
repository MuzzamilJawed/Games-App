...
function ImposterGame() {
  ...
  const [isReadyToReveal, setIsReadyToReveal] = useState<boolean>(false);
    ...
    <GameLayout
      ...
      actions={
        <>
          ...
          <button className="btn" onClick={() => startRound(1)}>
            New Game
          </button>
        </>
      }
    >
      <div className="imposter-game-container">
        ...
        {phase === "reveal" && activePlayer ? (
          <section className="imposter-center reveal-view">
            {!isReadyToReveal ? (
              <div className="pass-device-view">
                <div className="hero-icon">📱</div>
                <h3>Pass the device</h3>
                <p>Hand the device to the next player.</p>
                <button className="btn btn-lg" onClick={() => setIsReadyToReveal(true)}>
                  I am ready
                </button>
              </div>
            ) : (
              <>
                ...
                <button className="btn btn-secondary btn-next" onClick={advancePlayer} disabled={!isCardRevealed}>
                  {currentPlayerIndex + 1 === roles.length ? "All Roles Revealed" : "Done, Pass to Next"}
                </button>
              </>
            )}
          </section>
      ...
      {isSetupOpen ? (
        <div className="modal-backdrop" onClick={() => setIsSetupOpen(false)} role="presentation">
          <section className="setup-modal premium-modal" onClick={(event) => event.stopPropagation()}>
            <div className="setup-modal-head">
              <h3>Game Settings</h3>
              <button className="close-btn" onClick={() => setIsSetupOpen(false)}>&times;</button>
            </div>
            ...
            <div className="setup-grid">
              <div className="setup-section">
                ...
                <div className="settings-row">
                  <label className="settings-label">Total Players</label>
                ...
                <div className="settings-row" style={{ marginTop: '1rem' }}>
                  <label className="settings-label">Imposters</label>
                  <div className="number-input-group">
              ...
              <div className="setup-section custom-content-section">
                ...
                <div className="add-category-row">
                  ...
                  />
                  <button className="btn btn-sm" onClick={addNewCategory}>Add Category</button>
                </div>
                ...
                <div className="add-word-section">
                  ...
                  <div className="add-word-row">
                    <input 
                      placeholder="Add word to selected category..." 
                      value={newWord}
                      disabled={!addingToCategory}
                      onChange={e => setNewWord(e.target.value)}
                      onKeyPress={e => e.key === "Enter" && addNewWord(addingToCategory)}
                    />
                    <button 
            ...
            <div className="setup-footer">
              <button className="btn btn-lg btn-block" onClick={() => startRound(1)}>
                START GAME
              </button>
            </div>
          </section>
